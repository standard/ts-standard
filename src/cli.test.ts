...
import * as defaultOptionsLib from './options/default-options'
import * as packageOptionsLib from './options/package-options'
import * as cliOptionsLib from './options/cli-options'
...
describe('cli', () => {
  describe('printReport', () => {
    it('should use `standard` reporter to print the lint report to console', async (): Promise<void> => {
      ...
      const standardReporterSpy = jest
        .spyOn(standardReporterLib, 'standardReporter')
  ...
  describe('cli', () => {
    it('should log error if no project provided', async (): Promise<void> => {
      jest
      ...
      jest
        ...
        .mockReturnValue({} as any)
      jest.spyOn(cliOptionsLib, 'getCLIOptions').mockReturnValue({} as any)
    ...
    it('should call `lintFiles` function and `printReport` and exit with code 1 if errors found', async (): Promise<void> => {
      ...
      const printReportSpy = jest
        .spyOn(cliLib, 'printReport')
