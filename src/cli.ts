...
import {
  linter as Linter, LintReport, LintResult
...
export async function CLI (options: Options): Promise<void> {
...
Flags (advanced):
  ...
  try {
    report = await new Promise((resolve, reject) => {
      const cb = (err: Error | null, results: LintReport | null): void => {
        if (err != null) {
          return reject(err)
        }
        if (results == null) {
  ...
  const isFixable: boolean = report.results.some((res: any) => {
  ...
  const useFormatter = argv.formmatter ?? options.formatter
  let formatter: (reportResults: LintResult[]) => string
